			...
			for (int i = 0; i < nRxNodes; i++)
			{
				...
				if (rxNodeId != txNodeId)
				{
					...
					if (receiverMoving == 1)
					{
						...
						if(priority_rx>=PRIO_MANUEVER_THRESHOLD){
						...
						if (distSq > 0.0)
						{
							...
							for (int index = 1; index <= rangeCount; index++)
							{
								if (distSq <= m_txSafetyRangesSq[index - 1])
								{
									...
									// todo: index + base
									if(priority>=PRIO_MANUEVER_THRESHOLD){
							...
							for (int index = 1; index <= rangeCount; index++)
							{
								if (distSq <= m_txSafetyRangesSq[index - 1])
								{
									...
									if(priority>=PRIO_MANUEVER_THRESHOLD){
										...
										pkt_time=max_interval+(max_interval - min_interval - priority_rx * (max_interval - min_interval))/(max_priority- min_priority);
		...
		Ptr<Node> rxNode, int txNodeId, int rxNodeId, double prio, int MsgId, Ptr<Socket> socket)
{
	...
	double priority_threshold=PRIO_MANUEVER_THRESHOLD;
